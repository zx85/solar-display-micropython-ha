...
class HTTPServer(Server):
    def __init__(self, poller, local_ip):
        ...
        self.conns = dict()
        self.routes = {
            b"/": b"./include/captive_portal/index.html",
            b"/login": self.login,
        }
    ...
    def login(self, params):
        ...
        ha_url = unquote(params.get(b"ha_url", None))
        ha_token = unquote(params.get(b"ha_token", None))
        print(f"I got as far as the parameters - {ssid} {password} {ha_url} {ha_token}")
        # Write out credentials
        Creds(ssid=ssid, password=password, ha_url=ha_url, ha_token=ha_token).write()
